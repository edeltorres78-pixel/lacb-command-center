...
def kpi_dashboard_page():
    ...
    conn.close()
    df_manual = get_manual_tallies_df()

    owner_filter = st.selectbox("Owner Filter", ["Combined"] + OWNERS)
    time_filter = st.selectbox("Time Filter", ["Daily", "Weekly", "Monthly"])

    start_iso, end_iso = get_kpi_date_range(time_filter)
    ...
    if owner_filter != "Combined":
        ...
        if not df_manual.empty:
            df_manual = df_manual[df_manual["owner"] == owner_filter]

    if not df_tickets.empty:
        ticket_dates = df_tickets["created_at"].astype(str).str[:10]
        df_tickets = df_tickets[ticket_dates.between(start_iso, end_iso)]

    if not df_activities.empty:
        act_dates = df_activities["created_at"].astype(str).str[:10]
        df_activities = df_activities[act_dates.between(start_iso, end_iso)]

    if not df_manual.empty:
        tally_dates = df_manual["tally_date"].astype(str).str[:10]
        df_manual = df_manual[tally_dates.between(start_iso, end_iso)]

    st.subheader("Manual Communication Tally")
    with st.form("manual_tally_form"):
        t1, t2, t3, t4 = st.columns(4)
        tally_owner = t1.selectbox("Owner", OWNERS, key="manual_tally_owner")
        tally_date = t2.date_input("Date", value=date.today(), key="manual_tally_date")
        metric_type = t3.selectbox("Metric", MANUAL_TALLY_TYPES, key="manual_tally_metric")
        quantity = t4.number_input("Quantity", min_value=1, step=1, value=1, key="manual_tally_qty")
        notes = st.text_input("Notes (optional)", key="manual_tally_notes")
        submit_tally = st.form_submit_button("Add Manual Tally")

    if submit_tally:
        add_manual_tally(
            tally_date=tally_date.isoformat(),
            owner=tally_owner,
            metric_type=metric_type,
            quantity=int(quantity),
            notes=notes.strip(),
        )
        st.success("Manual tally added.")
        st.rerun()

    st.markdown("### Quick +/-1 Tally")
    quick_owner_default = owner_filter if owner_filter in OWNERS else OWNERS[0]
    q1, q2 = st.columns([1, 3])
    quick_owner = q1.selectbox("Quick Owner", OWNERS, index=OWNERS.index(quick_owner_default), key="manual_quick_owner")
    q2.caption("One-click add or subtract for today. Subtract is blocked below zero.")

    qa1, qa2, qa3 = st.columns(3)
    if qa1.button("Inbound RingCX +1", key="q_inbound_ringcx_plus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Inbound RingCX", 1, "Quick +1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()
    if qa2.button("Inbound RingCentral +1", key="q_inbound_ringcentral_plus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Inbound RingCentral", 1, "Quick +1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()
    if qa3.button("Outbound RingCX +1", key="q_outbound_ringcx_plus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Outbound RingCX", 1, "Quick +1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()

    qa4, qa5, qa6 = st.columns(3)
    if qa4.button("Outbound RingCentral +1", key="q_outbound_ringcentral_plus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Outbound RingCentral", 1, "Quick +1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()
    if qa5.button("SMS +1", key="q_sms_plus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "SMS", 1, "Quick +1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()
    if qa6.button("Email +1", key="q_email_plus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Email", 1, "Quick +1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()

    qb1, qb2, qb3 = st.columns(3)
    if qb1.button("Inbound RingCX -1", key="q_inbound_ringcx_minus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Inbound RingCX", -1, "Quick -1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()
    if qb2.button("Inbound RingCentral -1", key="q_inbound_ringcentral_minus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Inbound RingCentral", -1, "Quick -1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()
    if qb3.button("Outbound RingCX -1", key="q_outbound_ringcx_minus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Outbound RingCX", -1, "Quick -1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()

    qb4, qb5, qb6 = st.columns(3)
    if qb4.button("Outbound RingCentral -1", key="q_outbound_ringcentral_minus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Outbound RingCentral", -1, "Quick -1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()
    if qb5.button("SMS -1", key="q_sms_minus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "SMS", -1, "Quick -1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()
    if qb6.button("Email -1", key="q_email_minus"):
        ok, msg = apply_manual_tally_adjustment(today_str(), quick_owner, "Email", -1, "Quick -1 adjustment")
        st.success(msg) if ok else st.warning(msg)
        st.rerun()

    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Tickets Created", 0 if df_tickets.empty else len(df_tickets))
    c2.metric("Activity Logs Entered", 0 if df_activities.empty else len(df_activities))
    c3.metric("Outbound Calls", 0 if df_activities.empty else int(df_activities["action_type"].astype(str).str.contains("Outbound Call|\\bCall\\b", case=False, na=False, regex=True).sum()))
    c4.metric("Inbound Calls", 0 if df_activities.empty else int(df_activities["action_type"].astype(str).str.contains("Inbound Call", case=False, na=False).sum()))
    c5.metric("SMS Sent", 0 if df_activities.empty else int(df_activities["action_type"].astype(str).str.contains("SMS Sent|\\bSMS\\b", case=False, na=False, regex=True).sum()))
    st.metric("Emails Sent", 0 if df_activities.empty else int(df_activities["action_type"].astype(str).str.contains("Email Sent|\\bEmail\\b", case=False, na=False, regex=True).sum()))
    st.subheader("Manual Channel Totals")
    manual_totals = {m: 0 for m in MANUAL_TALLY_TYPES}
    if not df_manual.empty:
        grouped = df_manual.groupby("metric_type")["quantity"].sum().to_dict()
        for m in manual_totals:
            manual_totals[m] = int(grouped.get(m, 0))

    m1, m2, m3 = st.columns(3)
    m1.metric("Inbound RingCX", manual_totals["Inbound RingCX"])
    m2.metric("Inbound RingCentral", manual_totals["Inbound RingCentral"])
    m3.metric("Outbound RingCX", manual_totals["Outbound RingCX"])

    m4, m5, m6 = st.columns(3)
    m4.metric("Outbound RingCentral", manual_totals["Outbound RingCentral"])
    m5.metric("Manual SMS", manual_totals["SMS"])
    m6.metric("Manual Emails", manual_totals["Email"])

    st.subheader("Manual Tally Log")
