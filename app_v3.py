...
DB_PATH = "lacc_command_center_v3.db"
...
def next_customer_followup(attempt_no):
    if attempt_no >= 4:
        return add_business_days(date.today(), 2).isoformat()
    return add_business_days(date.today(), 1).isoformat()
...
def seed_demo(conn):
    ...
    demo = [
        ...
        ("CC-0002", "Email Sent to Vendor - ETA Request / Order Not in...", "Amber Margolis", "O-66211", "", "Supervisor", "Melisa", "Erika Sagasta", "Vendor", "Waiting on vendor", "Waiting on Vendor", 0, t.isoformat(), add_business_days(t, 1).isoformat(), 0, "", "", "Waiting on vendor ETA.", now_iso(), now_iso())
    ]
    conn.executemany("""
        INSERT INTO tickets (
            ...
            special_scheduling_required, scheduling_route, scheduling_reason, notes_summary, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, demo)
    conn.commit()
    ...
    if data.get("cc_id"):
        cur.execute("""
            UPDATE tickets SET
                ...
                issue_type=?, hubspot_stage=?, internal_status=?, attempt_no=?, last_activity_date=?, next_followup_date=?,
                special_scheduling_required=?, scheduling_route=?, scheduling_reason=?, notes_summary=?, updated_at=?
            WHERE cc_id=?
        """, (
            ...
            data["scheduling_route"], data["scheduling_reason"], data["notes_summary"], now_iso(), data["cc_id"]
        ))
    else:
        ...
        """, (
            ...
            data["assigned_by"], data["assigned_agent"], data["issue_type"], data["hubspot_stage"], data["internal_status"],
            int(data["attempt_no"]), data["last_activity_date"], data["next_followup_date"], int(bool(data["special_scheduling_required"])),
            data["scheduling_route"], data["scheduling_reason"], data["notes_summary"], now_iso(), now_iso()
        ))
        data["cc_id"] = cc_id
    conn.commit()
    conn.close()
...
def quick_log_page(agent_filter):
    ...
    with st.form("quick_log_form_v3"):
...
def scheduling_page():
    ...
    tab1, tab2 = st.tabs(["Scheduling Request Generator", "Installer / Region Helper"])
    with tab1:
        c1, c2 = st.columns(2)
    ...
    with tab2:
        st.caption("This tries to use Region_Map.csv if it exists in the same folder as the app.")
        region_map = load_region_map()
        if region_map.empty:
            st.warning("Region_Map.csv was not loaded. Put Region_Map.csv in the same folder as app_v3.py to use this helper.")
        else:
            st.success("Region_Map.csv loaded.")
            search = st.text_input("Search region map")
            if search:
                mask = pd.Series(False, index=region_map.index)
                for col in region_map.columns:
                    mask = mask | region_map[col].astype(str).str.contains(search, case=False, na=False)
                region_map = region_map[mask]
            st.dataframe(region_map, use_container_width=True, hide_index=True)
...
def history_page(agent_filter):
    ...
    c1, c2 = st.columns(2)
    with c1:
        df_download_button(acts, "LACB_Activity_History_V3.xlsx", "Download Activity History")
    with c2:
        df_download_button(tickets, "LACB_Ticket_Tracker_V3.xlsx", "Download Ticket Tracker")


def main():
    st.set_page_config(page_title="LACB Customer Care Command Center V3", layout="wide")
    init_db()
    st.title("LACB Customer Care Command Center V3")
    st.caption("Follow-ups, KPI tracking, ticket control, CRM notes, and scheduling support for Ed Torres and Erika Sagasta.")
    with st.sidebar:
        agent_filter = st.selectbox("Logged in as", ["All"] + AGENTS, index=1)
        page = st.radio("Navigation", ["Follow-Up Assistant", "KPI Dashboard", "Ticket Tracker", "Quick Log", "CRM Note Generator", "Scheduling Assistant", "History / Export"])
