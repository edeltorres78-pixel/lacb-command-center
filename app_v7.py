...
def extract_customer_name(text):
    m = re.search(r"CUSTOMER\s*:\s*([A-Z][A-Za-z\-\.' ]+)", text or "", flags=re.I)
...
def generate_sms(customer_name, body):
    return f"Hi {customer_name}, this is Ed from LA Custom Blinds. {body} You can text me here or call 1-800-533-7957 ext. 1111."


def generate_email(customer_name, order_no, body):
    return f"""Subject: LACB | Update | {order_no or 'Order'} | {customer_name}
...
def ticket_tracker_page(agent_filter):
    ...
    with st.expander("Create / Update Ticket", expanded=False):
        ...
        with st.form("ticket_form_v7"):
            ...
            issue_type = c5.selectbox("Issue Type", ISSUE_TYPES, key="ticket_issue_type")

            c6, c7, c8 = st.columns(3)
            ticket_source = c6.selectbox("Ticket Source", TICKET_SOURCES, key="ticket_ticket_source")
            assigned_by = c7.selectbox("Assigned By", ASSIGNED_BY, key="ticket_assigned_by")
            assigned_agent = c8.selectbox("Assigned Agent", AGENTS, key="ticket_assigned_agent")

            c9, c10, c11 = st.columns(3)
            hubspot_stage = c9.selectbox("HubSpot Stage", HUBSPOT_STAGES, key="ticket_hubspot_stage")
            internal_status = c10.selectbox("Internal Status", INTERNAL_STATUSES, key="ticket_internal_status")
            attempt_no = c11.number_input("Attempt #", min_value=0, max_value=4, key="ticket_attempt_no")

            c12, c13 = st.columns(2)
            last_activity_date = c12.date_input("Last Activity Date", key="ticket_last_activity_date")
            next_followup_date = c13.date_input("Next Follow-Up Date", key="ticket_next_followup_date")
            ...
            submitted = b1.form_submit_button("Save Ticket")
            save_start_new = b2.form_submit_button("Save + Start New")
            clear_form = b3.form_submit_button("Clear Form")
...
def quick_log_page(agent_filter):
    ...
    with st.form("quick_log_form_v7"):
...
def conversation_page():
    ...
    st.caption("Paste a call transcript, SMS thread, email thread, or internal notes, then extract ticket details.")
    st.session_state.setdefault("conv_text", "")
    source_default = st.selectbox("Conversation Source", ["Direct Call", "Email", "Direct SMS", "Internal Chat", "Receptionist"])
    st.session_state.conv_text = st.text_area("Paste transcript / messages / notes", value=st.session_state.conv_text, height=260)
    ...
    with c1:
        if st.button("Extract / Prefill Ticket Info"):
            ...
            st.success("Ticket fields detected and ready for Ticket Tracker.")
            st.json(prefill)
    with c2:
...
def output_builder_page(agent_filter):
    st.subheader("Ticket Output Builder")
    tickets = fetch_tickets()
    tickets = apply_ticket_filters(tickets, agent_filter, "All", "")
    if tickets.empty:
    ...
    options = {f'{r["cc_id"]} | {r["customer_name"]} | {r["order_no"] or ""} | {r["hubspot_title"]}': r["cc_id"] for _, r in tickets.iterrows()}
    selected = st.selectbox("Select ticket", list(options.keys()))
    ticket = tickets[tickets["cc_id"] == options[selected]].iloc[0]

    default_problem = f"{ticket['customer_name']} contacted me regarding order {ticket['order_no'] or 'N/A'} for {ticket['issue_type']}."
    default_resolution = ticket["notes_summary"] or "I reviewed the account, documented the activity, and provided the appropriate update."
    default_expectation = f"I will continue monitoring this ticket under status '{ticket['internal_status']}' and follow up on {ticket['next_followup_date'] or 'the next required step'}."
    default_sms_body = f"I wanted to provide you with an update regarding order {ticket['order_no'] or 'N/A'}. {ticket['notes_summary'] or 'I documented the latest activity and will continue monitoring the next step.'}"
    default_email_body = f"I wanted to provide you with an update regarding your order ({ticket['order_no'] or 'N/A'}).\n\n{ticket['notes_summary'] or 'I documented the latest activity and will continue monitoring the next step.'}"

    problem = st.text_area("Problem", value=default_problem, height=100)
    resolution = st.text_area("Resolution", value=default_resolution, height=140)
    expectation = st.text_area("Expectation", value=default_expectation, height=100)
    sms_body = st.text_area("Customer SMS body", value=default_sms_body, height=120)
    email_body = st.text_area("Customer Email body", value=default_email_body, height=150)

    if st.button("Generate Outputs"):
        st.text_area("HubSpot / CRM Summary", value=generate_ticket_summary(problem, resolution, expectation), height=260)
        st.text_area("CRM Note", value=generate_crm_note(ticket['customer_name'], ticket['order_no'], ticket['ticket_source'], ticket['issue_type'], resolution), height=220)
        st.text_area("Customer SMS", value=generate_sms(ticket['customer_name'], sms_body), height=140)
        st.text_area("Customer Email", value=generate_email(ticket['customer_name'], ticket['order_no'], email_body), height=260)
...
def history_page(agent_filter):
    ...
    c1, c2 = st.columns(2)
    with c1:
        df_download_button(acts, "LACB_Activity_History_V7.xlsx", "Download Activity History")
    with c2:
        df_download_button(tickets, "LACB_Ticket_Tracker_V7.xlsx", "Download Ticket Tracker")


def main():
    st.set_page_config(page_title="LACB Customer Care Command Center V7", layout="wide")
    init_db()
    st.title("LACB Customer Care Command Center V7")
    st.caption("Follow-ups, KPI tracking, ticket control, transcript intake, scheduling support, duplicate protection, service / repair building, and ticket output generation for Ed Torres and Erika Sagasta.")
    ...
    if page == "Follow-Up Assistant":
        ...
        conversation_page()
    elif page == "Ticket Output Builder":
        output_builder_page(agent_filter)
