...
def ticket_exists(order_no, hubspot_title, exclude_cc_id=None):
    ...
    if exclude_cc_id:
        cur.execute(
            "SELECT cc_id FROM tickets WHERE order_no = ? AND hubspot_title = ? AND cc_id <> ? LIMIT 1",
            (order_no, hubspot_title, exclude_cc_id),
        )
    else:
        cur.execute(
            "SELECT cc_id FROM tickets WHERE order_no = ? AND hubspot_title = ? LIMIT 1",
            (order_no, hubspot_title),
        )
    row = cur.fetchone()
    conn.close()
...
def get_installer_priority_for_region(region_value):
    ...
    installers = sorted(installers, key=lambda x: (priority_rank.get(x["priority"], 99), x["row_order"]))
    return installers
...
def generate_service_repair_template(original_order, repair_needed, why, photos_videos):
...
def reset_ticket_form_defaults():
    st.session_state["ticket_hubspot_title"] = ""
    st.session_state["ticket_customer_name"] = ""
    st.session_state["ticket_order_no"] = ""
    st.session_state["ticket_phone"] = ""
    st.session_state["ticket_issue_type"] = ISSUE_TYPES[0]
    st.session_state["ticket_ticket_source"] = TICKET_SOURCES[0]
    st.session_state["ticket_assigned_by"] = ASSIGNED_BY[0]
    st.session_state["ticket_assigned_agent"] = AGENTS[0]
    st.session_state["ticket_hubspot_stage"] = HUBSPOT_STAGES[0]
    st.session_state["ticket_internal_status"] = INTERNAL_STATUSES[0]
    st.session_state["ticket_attempt_no"] = 0
    st.session_state["ticket_last_activity_date"] = date.today()
    st.session_state["ticket_next_followup_date"] = date.today()
    st.session_state["ticket_scheduling_flags"] = []
    st.session_state["ticket_notes_summary"] = ""
...
def followup_page(agent_filter):
    ...
    def prep(df):
        ...
        out["recommended_next_action"] = df.apply(lambda r: recommended_next_action(r["internal_status"], int(r["attempt_no"] or 0)), axis=1)
        return out
...
def ticket_tracker_page(agent_filter):
    ...
    if "ticket_edit_select" not in st.session_state:
        st.session_state["ticket_edit_select"] = "Create New"

    with st.expander("Create / Update Ticket", expanded=False):
        editable = st.selectbox("Edit existing ticket", ["Create New"] + (tickets["cc_id"].tolist() if not tickets.empty else []), key="ticket_edit_select")
        row = None
        ...
        if row is not None:
            defaults = {
                "ticket_hubspot_title": row["hubspot_title"],
                "ticket_customer_name": row["customer_name"],
                "ticket_order_no": row["order_no"] or "",
                "ticket_phone": row["phone"] or "",
                "ticket_issue_type": row["issue_type"] if row["issue_type"] in ISSUE_TYPES else ISSUE_TYPES[0],
                "ticket_ticket_source": row["ticket_source"] if row["ticket_source"] in TICKET_SOURCES else TICKET_SOURCES[0],
                "ticket_assigned_by": row["assigned_by"] if row["assigned_by"] in ASSIGNED_BY else ASSIGNED_BY[0],
                "ticket_assigned_agent": row["assigned_agent"] if row["assigned_agent"] in AGENTS else AGENTS[0],
                "ticket_hubspot_stage": row["hubspot_stage"] if row["hubspot_stage"] in HUBSPOT_STAGES else HUBSPOT_STAGES[0],
                "ticket_internal_status": row["internal_status"] if row["internal_status"] in INTERNAL_STATUSES else INTERNAL_STATUSES[0],
                "ticket_attempt_no": int(row["attempt_no"]) if pd.notna(row["attempt_no"]) else 0,
                "ticket_last_activity_date": date.today() if not row["last_activity_date"] else datetime.strptime(row["last_activity_date"], "%Y-%m-%d").date(),
                "ticket_next_followup_date": date.today() if not row["next_followup_date"] else datetime.strptime(row["next_followup_date"], "%Y-%m-%d").date(),
                "ticket_scheduling_flags": [] if not row["scheduling_reason"] else [x.strip() for x in str(row["scheduling_reason"]).split(",") if x.strip() in SCHEDULING_FLAGS],
                "ticket_notes_summary": row["notes_summary"] or "",
            }
            for k, v in defaults.items():
                st.session_state[k] = v
        else:
            if "prefill_hubspot_title" in st.session_state:
                ...
                st.session_state.setdefault("ticket_assigned_by", ASSIGNED_BY[0])
                st.session_state.setdefault("ticket_assigned_agent", AGENTS[0])
                st.session_state.setdefault("ticket_hubspot_stage", HUBSPOT_STAGES[0])
                st.session_state.setdefault("ticket_internal_status", INTERNAL_STATUSES[0])
                st.session_state.setdefault("ticket_attempt_no", 0)
                st.session_state.setdefault("ticket_last_activity_date", date.today())
                st.session_state.setdefault("ticket_next_followup_date", date.today())
                st.session_state.setdefault("ticket_scheduling_flags", [])
                st.session_state.setdefault("ticket_notes_summary", "")
            elif "ticket_hubspot_title" not in st.session_state:
                reset_ticket_form_defaults()

        with st.form("ticket_form_v6"):
            c1, c2 = st.columns(2)
            hubspot_title = c1.text_input("HubSpot Ticket Title", key="ticket_hubspot_title")
            customer_name = c2.text_input("Customer Name", key="ticket_customer_name")

            c3, c4, c5 = st.columns(3)
            order_no = c3.text_input("Order #", key="ticket_order_no")
            phone = c4.text_input("Phone", key="ticket_phone")
            issue_type = c5.selectbox("Issue Type", ISSUE_TYPES, index=ISSUE_TYPES.index(st.session_state.get("ticket_issue_type", ISSUE_TYPES[0])), key="ticket_issue_type")

            c6, c7, c8 = st.columns(3)
            ticket_source = c6.selectbox("Ticket Source", TICKET_SOURCES, index=TICKET_SOURCES.index(st.session_state.get("ticket_ticket_source", TICKET_SOURCES[0])), key="ticket_ticket_source")
            assigned_by = c7.selectbox("Assigned By", ASSIGNED_BY, index=ASSIGNED_BY.index(st.session_state.get("ticket_assigned_by", ASSIGNED_BY[0])), key="ticket_assigned_by")
            assigned_agent = c8.selectbox("Assigned Agent", AGENTS, index=AGENTS.index(st.session_state.get("ticket_assigned_agent", AGENTS[0])), key="ticket_assigned_agent")

            c9, c10, c11 = st.columns(3)
            hubspot_stage = c9.selectbox("HubSpot Stage", HUBSPOT_STAGES, index=HUBSPOT_STAGES.index(st.session_state.get("ticket_hubspot_stage", HUBSPOT_STAGES[0])), key="ticket_hubspot_stage")
            internal_status = c10.selectbox("Internal Status", INTERNAL_STATUSES, index=INTERNAL_STATUSES.index(st.session_state.get("ticket_internal_status", INTERNAL_STATUSES[0])), key="ticket_internal_status")
            attempt_no = c11.number_input("Attempt #", min_value=0, max_value=4, key="ticket_attempt_no")

            c12, c13 = st.columns(2)
            last_activity_date = c12.date_input("Last Activity Date", value=st.session_state.get("ticket_last_activity_date", date.today()), key="ticket_last_activity_date")
            next_followup_date = c13.date_input("Next Follow-Up Date", value=st.session_state.get("ticket_next_followup_date", date.today()), key="ticket_next_followup_date")

            scheduling_flags = st.multiselect("Scheduling Flags", SCHEDULING_FLAGS, default=st.session_state.get("ticket_scheduling_flags", []), key="ticket_scheduling_flags")
            ...
            if submitted or save_start_new:
                duplicate_cc = ticket_exists(order_no, hubspot_title, exclude_cc_id=None if row is None else row["cc_id"])
                if duplicate_cc:
                    ...
                    if save_start_new:
                        reset_ticket_form_defaults()
                        st.success(f"Ticket saved: {cc_id}. Ready for new entry.")
                    else:
    ...
    dup_cc = st.selectbox("Select ticket to delete", [""] + (tickets["cc_id"].tolist() if not tickets.empty else []))
...
def quick_log_page(agent_filter):
    ...
    with st.form("quick_log_form_v6"):
...
def conversation_page():
    ...
    st.caption("Works for inbound calls, receptionist-created tickets, direct email/SMS/call tickets, and internal chat updates.")
    ...
    st.session_state.conv_text = st.text_area("Paste call transcript, SMS thread, email thread, or notes", value=st.session_state.conv_text, height=260)
    ...
    with c1:
        if st.button("Extract / Prefill Ticket Info"):
            ...
            st.session_state["prefill_ticket_source"] = source_default
    ...
    st.markdown("### Quick Ticket Title Preview")
    p_source = st.selectbox("Title Source", TICKET_SOURCES, key="title_source_preview")
    p_issue = st.selectbox("Title Issue Type", ISSUE_TYPES, key="title_issue_preview")
    p_order = st.text_input("Title Order #", key="title_order_preview")
    p_customer = st.text_input("Title Customer", key="title_customer_preview")
    if st.button("Generate Ticket Title"):
        st.text_input("Generated Ticket Title", value=generate_ticket_title(p_source, p_issue, p_order, p_customer), disabled=True)
...
def service_repair_builder_page():
    ...
    repair_needed_default = REPAIR_PRESETS[preset]["repair_needed"]
    why_default = REPAIR_PRESETS[preset]["why"]
    repair_needed = st.text_area("Repair Needed", value=repair_needed_default, height=120)
    why = st.text_area("Why", value=why_default, height=120)
    if st.button("Generate Service / Repair Template"):
        output = generate_service_repair_template(original_order, repair_needed, why, photos_videos)
        st.text_area("Template Output", value=output, height=220)
...
def history_page(agent_filter):
    ...
    c1, c2 = st.columns(2)
    with c1:
        df_download_button(acts, "LACB_Activity_History_V6.xlsx", "Download Activity History")
    with c2:
        df_download_button(tickets, "LACB_Ticket_Tracker_V6.xlsx", "Download Ticket Tracker")


def main():
    st.set_page_config(page_title="LACB Customer Care Command Center V6", layout="wide")
    init_db()
    st.title("LACB Customer Care Command Center V6")
    st.caption("Follow-ups, KPI tracking, ticket control, CRM notes, transcript intake, scheduling support, duplicate protection, and service/repair template building for Ed Torres and Erika Sagasta.")
    with st.sidebar:
