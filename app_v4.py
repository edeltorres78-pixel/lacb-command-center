# LACB Customer Care Command Center V4
import io
import os
import re
import sqlite3
...
DB_PATH = "lacc_command_center_v4.db"
...
TICKET_SOURCES = ["Receptionist", "Supervisor", "Sales Team", "Self-Created", "Direct SMS", "Direct Call", "Email", "Internal Chat"]
...
INTERNAL_STATUSES = ["Waiting on Customer", "Waiting on Vendor", "Waiting on DC", "Waiting on Internal Dept", "Scheduled", "Ready to Close", "Closed"]
ISSUE_TYPES = ["Service", "Shipping", "Vendor", "Install", "DC Request", "Payment", "Warranty", "Other"]
ACTIONS = ["Outbound Call", "Inbound Call", "SMS Sent", "Email Sent", "Internal Chat", "HubSpot Note Added", "QuoteRite Note Added", "Service Order Created", "Appointment Scheduled", "Scheduling Request Sent", "Vendor Follow-Up", "DC Follow-Up", "Internal Dept Follow-Up", "Ticket Closed"]
RESULTS = ["Spoke with customer", "No answer", "Left voicemail", "Waiting on customer", "Waiting on vendor", "Waiting on DC", "Waiting on internal department", "Resolved"]
SCHEDULING_FLAGS = ["Palm Springs", "Shutters", "Arizona", "2-Man Job", "Ladder >10 ft", "Ladder >20 ft", "Scaffolding"]
...
def seed_demo(conn):
    t = date.today()
    demo = [
        ("CC-0001", "Barry REQ Service Call - Denise Schmidt", "Denise Schmidt", "O-36379", "(714) 876-5969", "Receptionist", "Reception Desk", "Ed Torres", "Service", "Waiting on customer", "Waiting on Customer", 2, t.isoformat(), add_business_days(t, 1).isoformat(), 0, "Direct Scheduling", "Standard Service", "Customer needs service follow-up.", "", now_iso(), now_iso()),
        ("CC-0002", "Email Sent to Vendor - ETA Request", "Amber Margolis", "O-66211", "", "Supervisor", "Melisa", "Erika Sagasta", "Vendor", "Waiting on vendor", "Waiting on Vendor", 0, t.isoformat(), add_business_days(t, 1).isoformat(), 0, "", "", "Waiting on vendor ETA.", "", now_iso(), now_iso()),
    ]
    conn.executemany("""
        INSERT INTO tickets (
...
def get_installer_priority_for_region(region_value):
    ...
    for _, row in df.iterrows():
        ...
        if marker in {"P1", "P2", "P3"}:
            installers.append({"installer": str(row.get("NAME", "")).strip(), "priority": marker})
    return installers
...
def home_page(agent_filter):
    ...
    def prep(df):
        if df.empty:
            return df
        df = df[["cc_id", "hubspot_title", "customer_name", "order_no", "assigned_agent", "internal_status", "attempt_no", "last_activity_date", "next_followup_date"]].copy()
        df["recommended_next_action"] = df.apply(lambda r: recommended_next_action(r["internal_status"], int(r["attempt_no"] or 0)), axis=1)
        return df

    st.markdown("### Call First / Due Today")
    x = prep(due)
    st.dataframe(x, use_container_width=True, hide_index=True) if not x.empty else st.caption("No follow-ups due today.")

    c1, c2 = st.columns(2)
    with c1:
        st.markdown("### Overdue")
        x = prep(overdue)
        st.dataframe(x, use_container_width=True, hide_index=True) if not x.empty else st.caption("No overdue follow-ups.")
    with c2:
        st.markdown("### Ready to Close")
        x = prep(ready)
        st.dataframe(x, use_container_width=True, hide_index=True) if not x.empty else st.caption("No tickets ready to close.")
...
def ticket_tracker_page(agent_filter):
    ...
    with st.expander("Create / Update Ticket", expanded=False):
        ...
        with st.form("ticket_form_v4"):
            ...
            issue_type = c5.selectbox("Issue Type", ISSUE_TYPES, index=ISSUE_TYPES.index(row["issue_type"]) if row is not None and row["issue_type"] in ISSUE_TYPES else 0)
            c6, c7, c8 = st.columns(3)
            ticket_source = c6.selectbox("Ticket Source", TICKET_SOURCES, index=TICKET_SOURCES.index(row["ticket_source"]) if row is not None and row["ticket_source"] in TICKET_SOURCES else 0)
            assigned_by = c7.selectbox("Assigned By", ASSIGNED_BY, index=ASSIGNED_BY.index(row["assigned_by"]) if row is not None and row["assigned_by"] in ASSIGNED_BY else 0)
            assigned_agent = c8.selectbox("Assigned Agent", AGENTS, index=AGENTS.index(row["assigned_agent"]) if row is not None and row["assigned_agent"] in AGENTS else 0)
            c9, c10, c11 = st.columns(3)
            hubspot_stage = c9.selectbox("HubSpot Stage", HUBSPOT_STAGES, index=HUBSPOT_STAGES.index(row["hubspot_stage"]) if row is not None and row["hubspot_stage"] in HUBSPOT_STAGES else 0)
            internal_status = c10.selectbox("Internal Status", INTERNAL_STATUSES, index=INTERNAL_STATUSES.index(row["internal_status"]) if row is not None and row["internal_status"] in INTERNAL_STATUSES else 0)
            attempt_no = c11.number_input("Attempt #", min_value=0, max_value=4, value=int(row["attempt_no"]) if row is not None and pd.notna(row["attempt_no"]) else 0)
            c12, c13 = st.columns(2)
            last_activity_date = c12.date_input("Last Activity Date", value=date.today() if row is None or not row["last_activity_date"] else datetime.strptime(row["last_activity_date"], "%Y-%m-%d").date())
            next_followup_date = c13.date_input("Next Follow-Up Date", value=date.today() if row is None or not row["next_followup_date"] else datetime.strptime(row["next_followup_date"], "%Y-%m-%d").date())
            scheduling_flags = st.multiselect("Scheduling Flags", SCHEDULING_FLAGS, default=[] if row is None or not row["scheduling_reason"] else [x.strip() for x in str(row["scheduling_reason"]).split(",") if x.strip() in SCHEDULING_FLAGS])
    ...
    if filtered.empty:
        ...
        st.dataframe(filtered[["cc_id", "hubspot_title", "customer_name", "order_no", "assigned_agent", "internal_status", "attempt_no", "next_followup_date", "scheduling_route", "updated_at"]], use_container_width=True, hide_index=True)
...
def quick_log_page(agent_filter):
    ...
    with st.form("quick_log_form_v4"):
...
def conversation_page():
    st.subheader("Conversation Intake")
    st.caption("Paste call transcripts, SMS interactions, or email threads here. This page stores the raw conversation and helps prefill ticket basics. Full AI summarization inside the app would require an API connection in a later version.")
    tickets = fetch_tickets()
    options = ["No Ticket Selected"] + ([f'{r["cc_id"]} | {r["customer_name"]} | {r["hubspot_title"]}' for _, r in tickets.iterrows()] if not tickets.empty else [])
    selected = st.selectbox("Attach conversation to ticket", options)
    raw_text = st.text_area("Paste conversation / transcript / email thread", height=260)
    c1, c2 = st.columns(2)
    if c1.button("Extract Basics"):
        st.write({"Detected Order #": extract_order_no(raw_text), "Detected Phone": extract_phone(raw_text)})
    if c2.button("Save Raw Conversation") and selected != "No Ticket Selected":
        cc_id = selected.split(" | ")[0]
        row = tickets[tickets["cc_id"] == cc_id].iloc[0].to_dict()
        row["raw_conversation"] = raw_text
        save_ticket(row)
        st.success(f"Raw conversation saved to {cc_id}.")
...
def history_page(agent_filter):
    ...
    c1, c2 = st.columns(2)
    with c1:
        df_download_button(acts, "LACB_Activity_History_V4.xlsx", "Download Activity History")
    with c2:
        df_download_button(tickets, "LACB_Ticket_Tracker_V4.xlsx", "Download Ticket Tracker")


def main():
    st.set_page_config(page_title="LACB Customer Care Command Center V4", layout="wide")
    init_db()
    st.title("LACB Customer Care Command Center V4")
    st.caption("Follow-ups, KPI tracking, ticket control, CRM notes, transcript intake, and scheduling support for Ed Torres and Erika Sagasta.")
    with st.sidebar:
        agent_filter = st.selectbox("Logged in as", ["All"] + AGENTS, index=1)
        page = st.radio("Navigation", ["Follow-Up Assistant", "KPI Dashboard", "Ticket Tracker", "Quick Log", "Conversation Intake", "CRM Note Generator", "Scheduling Assistant", "History / Export"])
    if page == "Follow-Up Assistant":
        home_page(agent_filter)
